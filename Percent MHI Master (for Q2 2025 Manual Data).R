### Percent MHI MASTER - Q2 2025 Update ###
...
mhi <- read_excel("Manual Data/Q2 2025_08.28.25/MHI_ManualData_09.10.25.xlsx")
...
## CREATE PERCENT MHI INDICATOR ################################################
#### Join inventory with water rates data ####
inventory_wr <- sablplus %>% 
  as.data.frame() %>% 
  dplyr::select(!geometry) %>%
  rename(PWSID = SABL_PWSID) %>%
  left_join(wr %>% dplyr::select(PWSID, military_dummy, school_dummy, EAR23_STATUS, RATE_6HCF)) %>% 
  mutate(
  ...
  left_join(mhi %>% dplyr::select(PWSID, MHI))

#### Calculate Percent MHI ####

perc_mhi <- inventory_wr %>% 
  mutate(perc_mhi = (RATE_NUM*12)/MHI,
         perc_mhi = ifelse(is.na(perc_mhi) & RATE_6HCF == "Missing", "Missing", perc_mhi),
         perc_mhi = ifelse(is.na(perc_mhi) & RATE_6HCF == "N/A", "N/A", perc_mhi),
         MHI = scales::dollar(MHI),
         MHI = ifelse(is.na(MHI), "Missing", MHI))
#, accuracy = 0.01),

#### Clean Percent MHI ####

PERCENTMHI_CLEAN <- perc_mhi %>%
  dplyr::select(PWSID,
                NAME,
                `Monthly Total Drinking Water Charges for 6HCF` = RATE_6HCF,
                MHI,
                `Percent of Median Household Income (%MHI)` = perc_mhi)

#### Write Percent MHI ####
openxlsx::write.xlsx(PERCENTMHI_CLEAN,"C:/Users/rskillman/OneDrive - Water Boards/Documents/Data Work/Master Data Files/Manual Data/Q2 2025_08.28.25/PercentMHI_ManualData_09.10.25.xlsx") 
